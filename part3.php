...
<?php else: $recognized = !array_key_exists('recognized', $generated) || !empty($generated['recognized']); ?><article class="security-alert-card <?= !$recognized ? 'is-unrecognized' : (!empty($generated['critical']) ? 'is-critical' : 'is-normal') ?>"><span class="security-alert-label"><i class="bi <?= !$recognized ? 'bi-question-circle-fill' : (!empty($generated['critical']) ? 'bi-shield-exclamation' : 'bi-shield-check') ?>" aria-hidden="true"></i> <?= !$recognized ? 'Unrecognized Interface' : (!empty($generated['critical']) ? 'Security Alert' : 'Security Status') ?></span><div class="security-profile"><div class="security-profile-avatar"><i class="bi <?= $recognized ? 'bi-person-fill' : 'bi-question-lg' ?>" aria-hidden="true"></i></div><div><strong><?= $recognized ? 'Faculty User' : 'Unknown Output' ?></strong><span><?= escape((string) $generated['user_type']) ?></span><p><?= escape((string) $generated['message']) ?></p></div></div><h2><?= escape((string) $generated['severity']) ?></h2><dl><div><dt><i class="bi bi-key" aria-hidden="true"></i> Failed Login Attempts</dt><dd><?= $recognized ? (int) $generated['failed_attempts'] : '—' ?></dd></div><div><dt><i class="bi bi-lock-fill" aria-hidden="true"></i> Access Status</dt><dd><?= escape((string) $generated['access']) ?></dd></div><div><dt><i class="bi bi-bell-fill" aria-hidden="true"></i> Response</dt><dd><?= escape((string) $generated['response']) ?></dd></div></dl></article><?php endif; ?>
